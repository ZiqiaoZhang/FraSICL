...
#TODO(ailin): Add MultiTaskScaffoldRandomSplit for Binary Classification
class ScaffoldRandomSplitter(BasicSplitter):
    ...
    def BinaryMultiTaskMinorClassCount(self, dataset):
        ...
        return minorclass, minor_count, minor_ratio
    #TODO(ailin): add BinaryMultiTaskKeySplit
...
class ScaffoldSplitter(BasicSplitter):
    ...
    def split(self, dataset, opt):
        ...
        # sort the scaffolds keys by the number of the samples of each scaffolds.
        ...
        # larger scaffold_set first put into trainset, then to the validset and the test set.
...
raw_dataset = []
for i in range(15):
    raw_dataset.append({'SMILES': str(i), 'Value': '0'})
for i in range(15):
    raw_dataset.append({'SMILES': str(i+15), 'Value': '1'})

print(raw_dataset)
splitter = RandomSplitter()
(set1,set2, set3),(set1_idx, set2_idx, set3_idx) = splitter.split(raw_dataset, opt)
