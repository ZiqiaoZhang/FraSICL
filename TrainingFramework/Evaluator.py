...
class GeneralEvaluator(object):
    # Wrong! Only for evaluation with 1 sample per batch.
    # Not General At All!
...
class FraGATEvaluator(Evaluator):
    # module to perform evaluation for FraGAT model
    # It satisfies the special usage of FraGAT model during the evaluation step.
    def __init__(self, opt):
        super(FraGATEvaluator, self).__init__()
        self.opt = opt
    ...
    def eval(self, evalloader, model, metrics):
        ...
        for ii, data in enumerate(evalloader):
            ...
            # the sample provided by dataloader will add a new dimension 'batch' at dim=0, even if batch size is set to be 1.
            Label = Label.to(t.device(f"cuda:{self.opt.args['CUDA_VISIBLE_DEVICES']}" if t.cuda.is_available() else 'cpu'))  # [batch, batch(mol), task, 1]
            Label = Label.squeeze(-1)  # [batch, batch(mol), task]
            Label = Label.squeeze(0)  # [batch(mol), task]
            Label = Label.t()  # [task,batch(mol)]
            # for Label, different labels in a batch are actually the same, for they are exactly one molecule.
            # so the batch dim of Label is not exactly useful.
            output, _ = model(Input)  # [batch, output_size]
            output = output.mean(dim=0, keepdims=True)  # [1, output_size]

            for i in range(self.opt.args['TaskNum']):
                cur_task_output = output[:, i * self.opt.args['ClassNum']: (i + 1) * self.opt.args['ClassNum']]  # [1, ClassNum]
                cur_task_label = Label[i][0]  # all of the batch are the same, so only picking [i][0] is enough.
                if cur_task_label == -1:
                    continue
                else:
